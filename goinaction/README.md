...
### 3.2 导入
1. import导入包顺序 ---- 编译器首先查找Go的安装目录(GOROOT)，然后才会按照顺序查找GOPATH变量里列出的目录
...
##### 3.5.2 Go语言依赖管理工具
dep依赖管理工具学习，dep是官方版本。如果想系统学习dep依赖管理工具如何使用，请[参考博客](https://blog.csdn.net/thinking_fioa/article/details/89605937)
...
#### 4.1.5 在函数间传递数组
Go语言函数之间传递变量，总是以值的方式传递。如果这个变量是数组，不管多长，都会完整复制一份。所以数组作为函数参数传递，将严重影响程序的内存和性能。
